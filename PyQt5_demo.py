...
#        QMessageBox.information(self,"Info","Found Serialnumber: " + serienummer)
    ...
    def on_StartMeasBtn_clicked(self):
        ...
        # ret = AVS_MeasureCallback(globals.dev_handle, ctypes.addressof(callbackclass.callback(self, 0, 0)), nummeas)
        scans = 0
        while (scans < nummeas):
...
#        return False, 0
    ...
    def handle_newdata(self):
        ...
        for i in globals.spectraldata:
            print(i)
        print(len(globals.spectraldata))
        print(type(globals.spectraldata))


        #self.plot.update() #will only draw the last scan
        self.plot.repaint()
        return
